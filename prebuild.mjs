...
const CACHE_VER = 'v11';   /* поднять при любом изменении WBFIN_COLS — иначе кэш отдаст старые CSV */
...
const WBFIN_COLS = [
  ['Предмет'],
  ['Артикул поставщика'],
  /* Размер и баркод — для разреза выручки/заказов/выкупов ПО РАЗМЕРАМ (вкладка
     «Выручка»). В основной ключ модели НЕ входят: себестоимость, штуки поставки и
     справочник завязаны на «Предмет+Артикул» без размера, дробить их нельзя. */
  ['Размер', 'Size'],
  ['Баркод', 'Barcode', 'Штрихкод'],
  ['Тип документа'],
  ['Обоснование для оплаты'],
  ['Дата продажи'],
  /* Дата ЗАКАЗА покупателем — для режима «＋ по дате заказа»: выкупы привязываются
     к месяцу заказа, выкупаемость становится когортной. Без этой колонки parseWBFin
     не строит r.ordMs, и кнопка молча работает как обычный режим. */
  ...
  /* ЗАКАЗЫ живут на строках «Логистика»: «Количество доставок» с видом
     «К клиенту при продаже»/«К клиенту при отмене» = единиц отправлено покупателю.
     Проверено: (продажа+отмена) доставки, а выкуп = продажи. Привязка по «Дате продажи». */
  ['Количество доставок'],
  ['Виды логистики, штрафов и корректировок ВВ', 'Виды логистики'],
  ['Общая сумма штрафов'],
  ...
  /* Различает, УДЕРЖАН ли эквайринг из выплаты:
       «Перевыставление эквайринга» / «Компенсация платёжных услуг» → удержан
       «Комиссия за организацию платежа с НДС»                      → НЕ удержан, счёт отдельно */
...
/* эквайринг, выставляемый отдельным счётом (не вычитается из «К перечислению») */
...
const EN2RU_HEAD = {
  ...
  'Order Date by the Buyer':'Дата заказа покупателем',
  'Order Date':'Дата заказа покупателем',
  'Quantity':'Кол-во',
...
function xlsxToCSV(buf, fname) {
  ...
  const iEkvT = at('Тип платежа: компенсация платёжных услуг/Комиссия за интеграцию платёжных сервисов');
  ...
  for (let i = 1; i < out.length; i++) {
    ...
    const ekv = EKV_BILLED.test(String(r[iEkvT] || '')) ? 0 : N(r[iEkv]);   /* счёт отдельно → не удержан */
    comp += sg * (N(r[iVV]) + N(r[iNDS]) + ekv + N(r[iPvz]));
  }
  const gap = kom - comp;
