...
/* ── шаги разложения = каскад вкладки P&L (комиссия одной строкой = KOM.МП).
   Никакого разложения на ВВ/НДС/эквайринг/ПВЗ здесь нет — оно на вкладке «Комиссия».
   Тождество: Выкуп − Ком − Переменные − Доставка − Хранение − Налог + Компенсации = Пр.Опер;
              Пр.Опер − Реклама − Пост.Р = Пр.Факт (ровно формулы measures.js). ── */
function rzSteps(k){
  const st=[
    ...
    {l:'Комиссия МП',              v:-k.kom,      t:'sub', n:'как в P&L (КОМ.МП): реализовано − к перечислению · разбивка (ВВ/НДС/эквайринг/ПВЗ) — на вкладке «Комиссия»'},
