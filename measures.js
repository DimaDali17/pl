...
const CO_TXT={
  OZON:{
    ...
    nalog:{l:'Налог',t:'Налог УСН с базы «Выручка + Возврат выручки + Программы партнёров». Баллы за скидки в базу НЕ входят (база уже, чем Выкуп,руб). Ставка: 7% до 01.02.2026, далее 12%.'},
...
/* Колонки «без Баллов» вставляются СРАЗУ ПОСЛЕ своей парной колонки,
   а не блоком в конце: Выкуп,руб → Выкуп,руб ᴮᴮ, Ком.МП → Ком.МП ᴮᴮ и т.д. */
const NB_AFTER={vykr:'nbVykr',cena:'nbCena',kom:'nbKom',komP:'nbKomP',
                drl:'nbDrl',drr:'nbDrr',pribFact:'nbPribFact',pribFactP:'nbPribFactP'};
const COLS_NB=[
  COL('nbVykr','Выкуп,руб ᴮᴮ',fi,'Без баллов: только деньги, которые заплатил покупатель («Выручка» + «Возврат выручки»). Баллы за скидки исключены — их платит площадка.',{nb:1}),
  COL('nbCena','Ср.Чек ᴮᴮ',fi,'Средний чек по живым деньгам = Выкуп,руб ᴮᴮ ÷ Выкуп,шт.',{nb:1}),
  COL('nbKom','Ком.МП ᴮᴮ',fi,'Реальная комиссия = Вознаграждение − Баллы за скидки − Программы партнёров. Баллы площадка возвращает продавцу, поэтому фактическая нагрузка сильно ниже номинальных ~43%.',{nb:1}),
  COL('nbKomP','Ком% ᴮᴮ',fp1,'Реальная комиссия ÷ Выкуп,руб ᴮᴮ.',{nb:1}),
  COL('nbDrl','ДРЛ% ᴮᴮ',fp,'Доставка ÷ Выкуп,руб ᴮᴮ.',{nb:1}),
  COL('nbDrr','ДРР% ᴮᴮ',fp1,'Реклама ÷ Выкуп,руб ᴮᴮ.',{nb:1}),
  COL('nbPribFact','Пр.Факт ᴮᴮ',fi,'Чистая прибыль на реальной выручке = Выкуп,руб ᴮᴮ − Ком.МП ᴮᴮ − Переменные − Доставка − Хранение − Налог − Пост.Р − Акруалс − Реклама + Компенсации.',{nb:1}),
  COL('nbPribFactP','Ф% ᴮᴮ',fp,'Чистая прибыль ᴮᴮ ÷ Выкуп,руб ᴮᴮ.',{hl:1,nb:1})
];
